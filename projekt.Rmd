...
```
Dodatne povezave sem želela poiskati z metodo loess. Večinoma imamo podatke nekako zgoščene na območju, kjer je potrošnja med 25% in 30% BDP-ja ter rast med 0% in 5%, ampak nekatere točke močno odstopajo. Vseeno bi iz tega lahko predvidevali, da bo BDP konstantno rasel za 0-5%, če država za izobrazbo potroši nekje na intervalu med 25 in 30% BDP-ja. To je le sklepanje, veliko podatkov, ki bi prišli prav za to analizo, nisem upoštevala in ti bi lahko močno vplivali na rezultate.
```{r graf3, echo=FALSE, warning = FALSE, fig.align='center'}
graf3
```
Če si želimo ogledati še posamezno državo in si iz grafov za potrošnjo in rastjo BDP-ja ustvariti predstavo za povezavo, to lahko storimo z aplikacijo shiny. Lahko si izberemo še posebno vrsto izobrazbe in vsako primerjamo posebej.  
